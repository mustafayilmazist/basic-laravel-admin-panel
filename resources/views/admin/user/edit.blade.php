            ...
            <div class="col-lg-12">
                <div class="row">
                    ...
                    <div class="col-lg-6 text-right">
                        <a href="{{ route('user.index') }}" title="{{ __('Update user') }}">
        ...
        <div class="col-lg-12">
            ...
            <form method="POST" action="{{ route('user.update', $user->id) }}">
                ...
                <div class="form-group">
                    <label for="name"
                           class="{{$errors->has('name') ? 'text-red-400' : ''}}">{{ __('Name') }}</label>
                ...
                <div class="form-group">
                    <label for="email"
                           class="{{$errors->has('email') ? 'text-red-400' : ''}}">{{ __('Email') }}</label>
                ...
                <div class="form-group">
                    ...
                    <input id="password_confirmation"
                                        class="form-control{{$errors->has('password') ? 'border-red-400' : ''}}"
                                        type="password"
                        ...
                        @forelse ($roles as $role)
                            <div class="">
                                <label class="form-check-label">
                                    <input type="checkbox" name="roles[]" value="{{ $role->name }}" {{ in_array($role->id, $userHasRoles) ? 'checked' : '' }}>
                                    {{ $role->name }}
                                </label>
                            </div>
                        @empty
